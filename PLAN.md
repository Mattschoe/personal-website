...
a real `<img>` (`decoding="async"`, `object-fit:cover`, lazy by default but `eager` — `loading="eager"`
+ `fetchpriority="high"` — for above-the-fold heroes so the LCP image isn't deferred) from an item's `hero` and
